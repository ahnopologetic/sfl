...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        response.headers["X-RateLimit-Remaining"] = str(max(0, self.rate_limit - client["count"]))
...
def get_current_user(credentials: HTTPAuthorizationCredentials = Security(security)) -> uuid.UUID:
...
# Audio generation functions from server/main.py
...
@app.post("/users", response_model=ProfileResponse, status_code=status.HTTP_201_CREATED, tags=["users"])
async def create_user_profile(profile: ProfileCreate, user_id: uuid.UUID = Depends(get_current_user)):
...
@app.post("/snippets", response_model=SnippetJobResponse, status_code=status.HTTP_202_ACCEPTED, tags=["snippets"])
async def create_snippet_job(request: SnippetJobRequest, user_id: uuid.UUID = Depends(get_current_user)):
...
async def get_job_status(job_id: uuid.UUID, user_id: uuid.UUID = Depends(get_current_user)):
    job = jobs.get(job_id)
...
async def get_audio_snippet(snippet_id: uuid.UUID, user_id: uuid.UUID = Depends(get_current_user)):
    ...
    return FileResponse(
        ...
        filename=f"{snippet['title']}.mp3"
    )

@app.get("/snippets/{snippet_id}/metadata", response_model=SnippetMetadataResponse, tags=["snippets"])
async def get_snippet_metadata(snippet_id: uuid.UUID, user_id: uuid.UUID = Depends(get_current_user)):
    snippet = snippets.get(snippet_id)
...
@app.put("/snippets/{snippet_id}", response_model=SnippetUpdateResponse, tags=["snippets"])
