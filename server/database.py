...
class CuratedTopicsRepository:
    """Repository for curated topics operations."""

    @staticmethod
    async def get_curated_topics(user_id: uuid.UUID) -> List[Dict[str, Any]]:
        """Get all curated topics for a user."""
        result = (
            supabase_client.table("curated_topics")
            .select("*")
            .eq("user_id", str(user_id))
            .limit(20)
            .execute()
